...
function displayError(error, loc, offset ) {
	...
	link.rel = "stylesheet";
	link.type = "text/css";
	link.href = chrome.runtime.getURL("/css/content_error.css");
	...
	imgError.src = chrome.runtime.getURL("/img/error.gif");
...
function displayUI(theme, html) {
	...
	content += '<link rel="stylesheet" type="text/css" href="' + chrome.runtime.getURL("/css/json-core.css") + '">';
	...
	optionsElement.src = chrome.runtime.getURL("/img/options.png");
...
var Unicode = {
    ...
    show: function (str){
    	var statusElement, toolboxElement, expandElement, reduceElement, viewSourceElement, optionsElement, content = "";
			content += '<link rel="stylesheet" type="text/css" href="' + chrome.runtime.getURL("/css/json-core.css") + '">';
			content += '<style>.property { font-weight: bold; margin: 20px;}' + '</style>';
			content += '<div class="property">'+ str + '</div>';
			return content;
    }
};
...
function load() {
	var child, data;
	if (document.body 
		&& (document.body.childNodes[0] && document.body.childNodes[0].tagName == "PRE" 
	...
	{
		t = window.location.search.substring(1).replace(/^json=/, "");
		t = decodeURIComponent(t);

		r = /^\\[U|u]\w*/i;
		if(r.test(t))
		{
			t = Unicode.parse(t.toLowerCase());
			document.write(Unicode.show(t));	
		}
		else
		{
			data = extractData(t);
